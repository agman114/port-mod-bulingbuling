...
local states =
{
    ...
    State{
        ...
        onenter = function(inst)
            ...
            if equipped then 
                if equipped and equipped.PushEvent then equipped:PushEvent("startrowing", {owner = inst}) end
            end
           ...
           if inst.components.driver.vehicle.components.rowboatwakespawner then 
            ...
            if nextState ~= "row" and nextState ~= "sail" then 
                ...
                if nextState == "dismount" then --Make sure equipped items are pulled back out (only really for items with flames right now)
                    local equipped = inst.components.inventory:GetEquippedItem(EQUIPSLOTS.HANDS)
                    if equipped then
                        if equipped and equipped.PushEvent then equipped:PushEvent("stoprowing", {owner = inst}) end
                    end
                end 
            end 
        end,
