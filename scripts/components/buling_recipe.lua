...
function buling_task:Getitem()
	if taskitemlist and taskitemlist[self.tasknum] then
...
function buling_task:assault()
	local day = (TheWorld and TheWorld.state and TheWorld.state.cycles or 0)
	local tasktime = math.random(30, 360)
	if day % 10 == 0 and day ~= 0 and self.targettime == 0 and day > self.monsterday then
		self.targettime = self.buling_time + tasktime
		self.inst:DoTaskInTime(tasktime, function()
			if day > self.monsterday then
				self:dixi()
				self.monsterday = day
			end
		end)
...
function buling_task:dixi()
	...
	if (TheWorld and TheWorld.state and TheWorld.state.cycles or 0) <= 20 then
